...
// Fetch available confirmation slots
$sql_slots = "
    SELECT e.*, p.PriestName
    FROM UpcomingEvents e
    LEFT JOIN Priests p ON e.PriestID = p.PriestID
    WHERE e.SacramentType = 'Confirmation' AND e.Status = 'Available' AND e.EventDate >= CURDATE()
    ...
    echo '<script>
        document.addEventListener("DOMContentLoaded", function() {
            Swal.fire({
                icon: "success",
                title: "Success!",
                text: "Your Confirmation request has been successfully submitted. Please wait for approval.",
...
<head>
    ...
    <title>Sacraments Request - Confirmation Request</title>
...
<div class="flex">
    ...
    <h2 class="text-3xl font-semibold mb-6 text-gray-800">Confirmation Request</h2>
    ...
    <p class="text-lg text-gray-600 mb-8">Please fill out the required fields before you submit the Confirmation request and be sure to double check each field to complete the request.</p>
    ...
    <div class="form-container">
            <form action="confirmation_requestaccept.php" method="POST" enctype="multipart/form-data" class="space-y-6">
                <!-- Full Name, Father, Mother, Residence -->
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" class="form-control block w-full" id="fatherFullName" name="fatherFullName" required>
                    </div>
                    <div>
                        <label for="motherFullName" class="form-label">Mother's Full Name</label>
                        <input type="text" class="form-control block w-full" id="motherFullName" name="motherFullName" required>
                    </div>
                    <div>
                        <label for="residence" class="form-label">Residence</label>
                        <input type="text" class="form-control block w-full" id="residence" name="residence" required>
                    </div>
                </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
            <label for="romanCatholicCertificate" class="form-label">Roman Catholic Baptismal Certificate</label>
            <div class="relative">
                <input type="file" class="file-input pl-4 pr-10" id="romanCatholicCertificate" name="romanCatholicCertificate" accept=".png, .jpg, .jpeg, .pdf" required>
                <span class="upload-icon"><i class="fas fa-upload"></i></span>
            </div>
        </div>

        <!-- Confirmation/Parish Recommendation -->
        <div>
            <label for="confirmationRecommendation" class="form-label">Confirmation/Parish Recommendation</label>
            <div class="relative">
                <input type="file" class="file-input pl-4 pr-10" id="confirmationRecommendation" name="confirmationRecommendation" accept=".png, .jpg, .jpeg, .pdf" required>
                <span class="upload-icon"><i class="fas fa-upload"></i></span>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="relative">
                <input type="file" class="file-input pl-4 pr-10" id="baptismalCertificate" name="baptismalCertificate" accept=".png, .jpg, .jpeg, .pdf" required>
                <span class="upload-icon"><i class="fas fa-upload"></i></span>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="relative">
                <input type="file" class="file-input pl-4 pr-10" id="birthCertificate" name="birthCertificate" accept=".png, .jpg, .jpeg, .pdf" required>
                <span class="upload-icon"><i class="fas fa-upload"></i></span>
            </div>
        </div>
    </div>
 ...
 <div id="preScheduledSlots" class="mt-6 bg-white p-6 rounded-lg shadow-md">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4">Available Confirmation Slots</h3>
                    ...
                    <?php else: ?>
                        <p class="text-red-600">No available confirmation slots at the moment. Please check the calendar from time to time and request later.</p>
                    <?php endif; ?>
                </div>
    ...
    <button type="button" class="btn btn-primary" onclick="showConfirmationModal()">Submit Request</button>
        ...
        <div id="successModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
            <div class="bg-white rounded-lg shadow-lg p-8 w-full max-w-md text-center">
                ...
                <p class="text-gray-700 mb-6">Your confirmation request has been successfully submitted. Please wait for approval.</p>
