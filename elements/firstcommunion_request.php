...
// Fetch available First Communion slots
$sql_slots = "
    SELECT e.*, p.PriestName
    FROM UpcomingEvents e
    LEFT JOIN Priests p ON e.PriestID = p.PriestID
    WHERE e.SacramentType = 'First Communion' AND e.Status = 'Available' AND e.EventDate >= CURDATE()
    ...
    echo '<script>
        document.addEventListener("DOMContentLoaded", function() {
            Swal.fire({
                icon: "success",
                title: "Success!",
                text: "Your First Communion request has been successfully submitted. Please wait for approval.",
...
<head>
    ...
    <title>Sacraments Request - First Communion Request</title>
...
.upload-icon.uploaded {
    color: #28a745; /* Green icon when file is uploaded */
}
...
<div class="flex">
    ...
    <i class="fas fa-arrow-left"></i> Back to Sacraments
        ...
        <h2 class="text-3xl font-semibold mb-6 text-gray-800">First Communion Request</h2>
        ...
        <!-- Overview Text -->
        <p class="text-lg text-gray-600 mb-8">Please fill out the required fields before you submit the First Communion request and be sure to double check each information of the fields to complete the request.</p>
        ...
        <div class="form-container">
            <form action="firstcommunion_requestaccept.php" method="POST" enctype="multipart/form-data" class="space-y-6">
                <!-- Personal Information of the Child -->
                <h3 class="text-xl font-bold mb-4 text-gray-800">Personal Information of the Child</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="fullNameChild" class="form-label">Full Name of the Child</label>
                        <input type="text" class="form-control block w-full" id="fullNameChild" name="fullNameChild" >
                    </div>
                    <div>
                        <label for="dateOfBirth" class="form-label">Date of Birth</label>
                        <input type="date" class="form-control block w-full" id="dateOfBirth" name="dateOfBirth" >
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="placeOfBirth" class="form-label">Place of Birth</label>
                        <input type="text" class="form-control block w-full" id="placeOfBirth" name="placeOfBirth" >
                    </div>
                    <div>
                        <label for="baptismalDate" class="form-label">Baptismal Date</label>
                        <input type="date" class="form-control block w-full" id="baptismalDate" name="baptismalDate" >
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="baptismalParish" class="form-label">Baptismal Parish</label>
                        <input type="text" class="form-control block w-full" id="baptismalParish" name="baptismalParish" >
                    </div>
                    <div>
                        <label for="fatherName" class="form-label">Father's Name</label>
                        <input type="text" class="form-control block w-full" id="fatherName" name="fatherName" >
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="motherName" class="form-label">Mother's Name (Maiden Name)</label>
                        <input type="text" class="form-control block w-full" id="motherName" name="motherName" >
                    </div>
                </div>

                <!-- Parent/Guardian Contact Information -->
                <h3 class="text-xl font-bold mb-4 text-gray-800">Parent/Guardian Contact Information</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="parentGuardianName" class="form-label">Name of Parent/Guardian</label>
                        <input type="text" class="form-control block w-full" id="parentGuardianName" name="parentGuardianName" >
                    </div>
                    <div>
                        <label for="phoneNumber" class="form-label">Phone Number</label>
                        <input type="text" class="form-control block w-full" id="phoneNumber" name="phoneNumber" >
                    </div>
                </div>
               <div>
               <div>
    ...
    <input type="email" class="form-control block w-full md:w-1/2" id="emailAddress" name="emailAddress" >
</div>
<br>
             ...
             <!-- Document Uploads -->
<h3 class="text-xl font-semibold mb-4 text-red-400">Upload Required Documents</h3>
<p class="text-gray-600 mb-4">Please attach the following documents:</p>
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">

    <!-- Baptismal Certificate -->
    <div>
        <label for="baptismalCertificate" class="form-label">Baptismal Certificate</label>
        <div class="relative file-upload">
            <input type="file" class="file-input pl-4 pr-10" id="baptismalCertificate" name="baptismalCertificate" accept=".png, .jpg, .jpeg, .pdf" >
            <span class="upload-icon absolute right-4 top-1/2 transform -translate-y-1/2"><i class="fas fa-upload"></i></span>
        </div>
    </div>

    <!-- Certification from BEC -->
    <div>
        <label for="becCertification" class="form-label">Certification from BEC (Basic Ecclesial Community)</label>
        <div class="relative file-upload">
            <input type="file" class="file-input pl-4 pr-10" id="becCertification" name="becCertification" accept=".png, .jpg, .jpeg, .pdf" >
            <span class="upload-icon absolute right-4 top-1/2 transform -translate-y-1/2"><i class="fas fa-upload"></i></span>
        </div>
    </div>
</div>
...
<div class="mt-4">
    ...
    <div class="relative file-upload">
        <input type="file" class="file-input pl-4 pr-10" id="proofOfAddress" name="proofOfAddress" accept=".png, .jpg, .jpeg, .pdf" >
        <span class="upload-icon absolute right-4 top-1/2 transform -translate-y-1/2"><i class="fas fa-upload"></i></span>
    </div>
</div>
   <!-- Available Slots Section -->
   <div id="preScheduledSlots" class="mt-6 bg-white p-6 rounded-lg shadow-md">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4">Available First Communion Slots</h3>
                    ...
                    <?php else: ?>
                        <p class="text-red-600">No available First Communion slots at the moment. Please check back later.</p>
                    <?php endif; ?>
                </div>
        ...
        <div id="successModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden">
            <div class="bg-white rounded-lg shadow-lg p-8 w-full max-w-md text-center">
                ...
                <p class="text-gray-700 mb-6">Your First Communion request has been successfully submitted. Please wait for approval.</p>
    ...
    document.querySelectorAll('.file-input').forEach(input => {
        input.addEventListener('change', function () {
            if (this.files.length > 0) {
                ...
                this.nextElementSibling.classList.add('uploaded'); // Adds green icon when file is selected
            } else {
